...
export default function AppointmentModal({
  ...
  selectedDate,
  currentUser,
}: AppointmentModalProps) {  const [doctors, setDoctors] = useState<Doctor[]>([]);
  const [patients, setPatients] = useState<Patient[]>([]);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);  // Form state
  ...
  useEffect(() => {
    if (visible) {
      loadUsersData();      if (appointment) {
        // Editing existing appointment
        setFormData({
          appointment_date: appointment.appointment_date ? moment(appointment.appointment_date).format('YYYY-MM-DD') : selectedDate || moment().format('YYYY-MM-DD'),
          appointment_time: appointment.appointment_time ? moment(appointment.appointment_time, 'HH:mm:ss').format('HH:mm') : '09:00',
  ...
  }, [visible, appointment, selectedDate, currentUser]);
  ...
  const loadUsersData = async () => {
    ...
    try {
      ...
      if (currentUser?.role !== 'patient') {
        console.log('🔍 Loading patients - current user role:', currentUser?.role);
        console.log('🔍 Making request to:', `${API_BASE_URL}/api/users/patients/`);
    ...
    try {
      ...
      Alert.alert(
        'Demo Mode',
        `Backend not connected. In a real environment, this appointment would be ${appointment ? 'updated' : 'created'}.`,
        [{ text: 'OK', onPress: () => { onSave(); onClose(); } }]
      );
    } finally {
      setSaving(false);
          ...
          {loading ? (
            ...
            <>              <ThemedView style={styles.section}>
                ...
                <ThemedText style={styles.dateText}>
                  {formData.appointment_date ? moment(formData.appointment_date).format('MMMM D, YYYY') : 'No date selected'}
                </ThemedText>
              </ThemedView>              <ThemedView style={styles.section}>
                <ThemedText style={styles.label}>Time</ThemedText>
                <ThemedView style={styles.pickerContainer}>                  <Picker
              ...
              <ThemedView style={styles.section}>
                ...
                <ThemedView style={styles.pickerContainer}>                  <Picker
                    ...
                    onValueChange={(value) => {
                      console.log('Duration picker value:', value, typeof value);
              ...
              {currentUser?.role !== 'patient' && (
                <ThemedView style={styles.section}>
                  <ThemedText style={styles.label}>Patient</ThemedText>                  {patients.length > 0 ? (
                    <ThemedView style={styles.pickerContainer}>                      <Picker
                        selectedValue={formData.patient_id || 0}
                        onValueChange={(value) => {
                          console.log('Patient picker value:', value, typeof value);
                      ...
                      >
                        ...
                        {patients.map(patient => {
                          const patientLabel = patient.first_name && patient.last_name 
                            ...
                            : patient.username ? patient.username : `Patient ${patient.id}`;
                          return (
                            <Picker.Item
                              key={patient.id}
                              label={patientLabel}
              ...
              {currentUser?.role !== 'doctor' && (
                <ThemedView style={styles.section}>
                  <ThemedText style={styles.label}>Doctor</ThemedText>                  {doctors.length > 0 ? (
                    <ThemedView style={styles.pickerContainer}>                      <Picker
